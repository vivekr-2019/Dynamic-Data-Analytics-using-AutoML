...
def index():
    ...
    if request.method == 'POST':
        ...
        predictions['Random_Forest'] = 'Safe' if make_prediction(input_data_list, Random_Forest_Algorithm) == 0 else 'Malicious'
        predictions['k_nearest_neighbor'] = 'Safe' if make_prediction(input_data_list, k_nearest_neighbor_Algorithm) == 0 else 'Malicious'
        predictions['naive_bayes'] = 'Safe' if make_prediction(input_data_list, Naive_Bayes_Algorithm) == 0 else 'Malicious'
        predictions['ANN'] = 'Safe' if make_prediction(input_data_list, Naive_Bayes_Algorithm) == 0 else 'Malicious'
        predictions['LGBM_classifier'] = 'Safe' if make_prediction(input_data_list, LGBM_Classifier_Algorithm) == 0 else 'Malicious'
